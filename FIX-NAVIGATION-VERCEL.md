...
Sur https://rankwell.vercel.app/, **36 éléments sur 82 restent invisibles** (opacity: 0, blur: 16px) après navigation entre pages. Le contenu existe dans le DOM mais n'est pas visible.

### Cause Racine

Les animations `RevealAnimation` avec `gsap.from()` définissent les éléments comme invisibles AVANT l'animation. Quand un élément est déjà dans le viewport après navigation, le ScrollTrigger ne se déclenche jamais → l'élément reste invisible.
