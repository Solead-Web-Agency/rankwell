/**
 * PAGE - Agence GEO & SEO à Poitiers
 * URL: /agence-geo-seo/france/poitiers (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function PoitiersPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/poitiers',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Poitiers',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/poitiers',
        }}
      />
