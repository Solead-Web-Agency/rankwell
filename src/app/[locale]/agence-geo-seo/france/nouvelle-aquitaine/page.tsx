/**
 * PAGE - Agence GEO & SEO en Nouvelle-Aquitaine
 * URL: /agence-geo-seo/france/nouvelle-aquitaine (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function NouvelleAquitainePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/nouvelle-aquitaine',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO en Nouvelle-Aquitaine',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/nouvelle-aquitaine',
        }}
      />
