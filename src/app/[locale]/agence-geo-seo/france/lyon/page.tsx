/**
 * PAGE - Agence GEO & SEO pour les entreprises lyonnaises
...
// PAGE COMPONENT
// ============================================
export default async function LyonPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/lyon',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO pour les entreprises lyonnaises',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/lyon',
        }}
      />
      <FAQJsonLd faqs={faqSchemaItems} />
      {/* Pas de LocalBusinessJsonLd - pas de bureau à Lyon */}
