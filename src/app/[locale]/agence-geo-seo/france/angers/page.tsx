/**
 * PAGE - Agence GEO & SEO à Angers
 * URL: /agence-geo-seo/france/angers (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function AngersPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/angers',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Angers',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/angers',
        }}
      />
