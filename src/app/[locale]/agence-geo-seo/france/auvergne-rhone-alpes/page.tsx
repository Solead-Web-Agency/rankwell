/**
 * PAGE - Agence GEO & SEO en Auvergne-Rhône-Alpes
...
// PAGE COMPONENT
// ============================================
export default async function AuvergneRhoneAlpesPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/auvergne-rhone-alpes',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO en Auvergne-Rhône-Alpes',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/auvergne-rhone-alpes',
        }}
      />
