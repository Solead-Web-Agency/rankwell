...
export default async function ValencePage({ params }: PageProps) {
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/valence',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Valence',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/valence',
        }}
      />
