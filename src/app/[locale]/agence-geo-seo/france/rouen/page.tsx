...
// PAGE COMPONENT
// ============================================
export default async function RouenPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/rouen',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Rouen',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/rouen',
        }}
      />
