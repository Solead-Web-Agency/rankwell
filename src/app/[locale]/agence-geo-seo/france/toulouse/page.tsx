/**
 * PAGE - Agence GEO & SEO à Toulouse
 * URL: /agence-geo-seo/france/toulouse (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function ToulousePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/toulouse',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Toulouse',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/toulouse',
        }}
      />
