/**
 * PAGE - Agence GEO & SEO à Marseille
 * URL: /agence-geo-seo/france/marseille (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function MarseillePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/marseille',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Marseille',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/marseille',
        }}
      />
