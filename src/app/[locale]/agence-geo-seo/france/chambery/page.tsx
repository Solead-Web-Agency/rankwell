/**
 * PAGE - Agence GEO & SEO à Chambéry
...
// PAGE COMPONENT
// ============================================
export default async function ChamberyPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/chambery',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Chambéry',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/chambery',
        }}
      />
