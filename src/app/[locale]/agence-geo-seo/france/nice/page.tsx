/**
 * PAGE - Agence GEO & SEO à Nice
 * URL: /agence-geo-seo/france/nice (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function NicePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/nice',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Nice',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/nice',
        }}
      />
