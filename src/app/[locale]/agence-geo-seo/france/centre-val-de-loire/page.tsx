/**
 * PAGE - Agence GEO & SEO en Centre-Val de Loire
...
// PAGE COMPONENT
// ============================================
export default async function CentreValDeLoirePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/centre-val-de-loire',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO en Centre-Val de Loire',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/centre-val-de-loire',
        }}
      />
