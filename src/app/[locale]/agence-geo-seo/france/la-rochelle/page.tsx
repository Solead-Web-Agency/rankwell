/**
 * PAGE - Agence GEO & SEO à La Rochelle
 * URL: /agence-geo-seo/france/la-rochelle (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function LaRochellePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/la-rochelle',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à La Rochelle',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/la-rochelle',
        }}
      />
