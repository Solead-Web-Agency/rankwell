/**
 * PAGE - Agence GEO & SEO à Cannes
 * URL: /agence-geo-seo/france/cannes (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function CannesPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/cannes',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Cannes',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/cannes',
        }}
      />
