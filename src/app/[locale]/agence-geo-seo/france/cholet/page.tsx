/**
 * PAGE - Agence GEO & SEO à Cholet
 * URL: /agence-geo-seo/france/cholet (FR only)
 */
...
export default async function CholetPage({ params }: PageProps) {
  const { locale } = await params;
  if (locale !== 'fr') notFound();

  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/cholet',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd service={{ name: 'Agence GEO et SEO à Cholet', description: data.metadata.description, url: 'https://www.rankwell.agency/agence-geo-seo/france/cholet' }} />
