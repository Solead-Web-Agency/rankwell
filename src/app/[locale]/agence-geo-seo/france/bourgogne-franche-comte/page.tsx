/**
 * PAGE - Agence GEO & SEO en Bourgogne-Franche-Comté
...
// PAGE COMPONENT
// ============================================
export default async function BourgogneFrancheComtePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/bourgogne-franche-comte',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO en Bourgogne-Franche-Comté',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/bourgogne-franche-comte',
        }}
      />
