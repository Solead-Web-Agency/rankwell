/**
 * PAGE - Agence GEO & SEO à Montpellier
 * URL: /agence-geo-seo/france/montpellier (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function MontpellierPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/montpellier',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Montpellier',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/montpellier',
        }}
      />
