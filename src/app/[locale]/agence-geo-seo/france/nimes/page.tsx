/**
 * PAGE - Agence GEO & SEO à Nîmes
 * URL: /agence-geo-seo/france/nimes (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function NimesPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/nimes',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Nîmes',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/nimes',
        }}
      />
