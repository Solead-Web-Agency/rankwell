/**
 * PAGE - Agence GEO & SEO en Corse
 * URL: /agence-geo-seo/france/corse (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function CorsePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/corse',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO en Corse',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/corse',
        }}
      />
