/**
 * PAGE - Agence GEO & SEO à Perpignan
 * URL: /agence-geo-seo/france/perpignan (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function PerpignanPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/perpignan',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Perpignan',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/perpignan',
        }}
      />
