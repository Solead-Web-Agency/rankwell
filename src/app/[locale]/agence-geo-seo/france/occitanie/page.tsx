/**
 * PAGE - Agence GEO & SEO en Occitanie
 * URL: /agence-geo-seo/france/occitanie (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function OccitaniePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/occitanie',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO en Occitanie',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/occitanie',
        }}
      />
