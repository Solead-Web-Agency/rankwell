/**
 * PAGE - Agence GEO & SEO à Limoges
 * URL: /agence-geo-seo/france/limoges (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function LimogesPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/limoges',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Limoges',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/limoges',
        }}
      />
