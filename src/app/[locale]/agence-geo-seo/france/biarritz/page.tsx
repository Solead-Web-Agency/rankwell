/**
 * PAGE - Agence GEO & SEO à Biarritz
 * URL: /agence-geo-seo/france/biarritz (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function BiarritzPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/biarritz',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Biarritz',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/biarritz',
        }}
      />
