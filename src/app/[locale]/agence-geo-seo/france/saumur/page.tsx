...
export default async function SaumurPage({ params }: PageProps) {
  const { locale } = await params;
  if (locale !== 'fr') notFound();

  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/saumur',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd service={{ name: 'Agence GEO et SEO à Saumur', description: data.metadata.description, url: 'https://www.rankwell.agency/agence-geo-seo/france/saumur' }} />
