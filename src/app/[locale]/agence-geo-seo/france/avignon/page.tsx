/**
 * PAGE - Agence GEO & SEO à Avignon
 * URL: /agence-geo-seo/france/avignon (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function AvignonPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/avignon',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Avignon',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/avignon',
        }}
      />
