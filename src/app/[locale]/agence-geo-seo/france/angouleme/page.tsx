/**
 * PAGE - Agence GEO & SEO à Angoulême
 * URL: /agence-geo-seo/france/angouleme (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function AngoulemePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/angouleme',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Angoulême',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/angouleme',
        }}
      />
