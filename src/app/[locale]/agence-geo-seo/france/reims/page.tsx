...
// PAGE COMPONENT
// ============================================
export default async function ReimsPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/reims',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Reims',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/reims',
        }}
      />
