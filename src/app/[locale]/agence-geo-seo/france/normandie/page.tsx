/**
 * PAGE - Agence GEO & SEO en Normandie
...
// PAGE COMPONENT
// ============================================
export default async function NormandiePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/normandie',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO en Normandie',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/normandie',
        }}
      />
