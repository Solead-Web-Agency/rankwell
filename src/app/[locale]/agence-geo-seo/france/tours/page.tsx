...
export default async function ToursPage({ params }: PageProps) {
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/tours',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Tours',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/tours',
        }}
      />
