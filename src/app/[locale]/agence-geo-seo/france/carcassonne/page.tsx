/**
 * PAGE - Agence GEO & SEO à Carcassonne
 * URL: /agence-geo-seo/france/carcassonne (FR only)
 *
...
// PAGE COMPONENT
// ============================================
export default async function CarcassonnePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbSchemaItems = data.heroData.breadcrumb.map((item) => ({
    name: item.label,
    url: item.href || 'https://www.rankwell.agency/agence-geo-seo/france/carcassonne',
  }));
  ...
  return (
    <>
      ...
      <ServiceJsonLd
        service={{
          name: 'Agence GEO et SEO à Carcassonne',
          description: data.metadata.description,
          url: 'https://www.rankwell.agency/agence-geo-seo/france/carcassonne',
        }}
      />
